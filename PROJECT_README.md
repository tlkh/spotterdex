...
| --- | --- | --- |
| Home | Timothy Liu's selected aviation photographs | Open the curated photographs in the shared viewer or continue into the archive. |
...
Top-level pages are `index.html` (Home), `map.html`, `aircraft-dex.html`, `squadrons.html`, `airshows.html`, and `stats.html`. All load the same shared core bundle; the Stats page loads EXIF separately, and the viewer hydrates the full manifest when needed. On mobile, Squadrons and Stats are available through More in the five-item tab bar.
...
- `index.html#work=<selected-photo-id>`
...
The v2 migration is intentionally a clean break for entity IDs. Existing `index.html#location=...` and `index.html#photo=...` URLs redirect to the corresponding map route so shared links continue to work after Home replaces the map at the root.
