...
`service-worker.js` provides the installable offline shell declared by `manifest.webmanifest`. Generated catalog payloads are fetched network-first so a deploy is visible immediately; the precached shell uses stale-while-revalidate; thumbnails and photos use capped runtime caches. Both cache-version strings are stamped by the builder from a hash of the precached assets and the image profile, so they should never be edited by hand.
