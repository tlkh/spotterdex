...
SHARE_PAGE_CSS = (
    ...
    ".sp-write-up{margin:0 0 24px;padding:18px 20px;background:#211e1b;border:1px solid #37322d;border-radius:0;max-width:78ch}"
    ...
    "font-weight:600;padding:12px 22px;border-radius:0}"
...
def build_database_catalog(args: argparse.Namespace, root: Path, warnings: BuildWarningLog) -> int:
    ...
    resolved_generated_at = stable_generated_at(full_manifest["generatedAt"], full_manifest, json_output)
    full_manifest["generatedAt"] = resolved_generated_at
    legacy_manifest["generatedAt"] = resolved_generated_at
    ...
    write_text_if_changed(json_output, json.dumps(full_manifest, indent=2, ensure_ascii=True) + "\n")
    ...
    worker_path = stamp_service_worker(
        root,
        media_profile=(
            f"w{int(args.width)}-t{int(args.thumb_width)}"
            f"-q{int(args.jpeg_quality)}-tq{int(args.thumb_jpeg_quality)}"
            f"-{FULL_JPEG_PROFILE}-{THUMB_JPEG_PROFILE}"
        ),
        warnings=warnings,
    )
    warnings.print()
...
def write_text_if_changed(path: Path, text: str) -> bool:
    """Write only when content differs so repeat builds leave the tree clean."""
    if path.is_file():
        try:
...
def stable_generated_at(candidate: str, manifest: Dict[str, Any], previous_output: Path) -> str:
    """Keep the previous timestamp when the payload is otherwise unchanged.

    Rebuilding an untouched catalog would otherwise rewrite every generated file
    and every sitemap ``lastmod``, which buries real changes in diff noise and
    tells crawlers the whole site changed.
    """
    if not previous_output.is_file():
        return candidate
    try:
        previous = json.loads(previous_output.read_text("utf-8"))
    ...
    if not isinstance(previous_generated_at, str) or not previous_generated_at:
        return candidate
    if dict(manifest, generatedAt=previous_generated_at) == previous:
        return previous_generated_at
    return candidate


def stamp_service_worker(root: Path, media_profile: str, warnings: BuildWarningLog) -> Optional[Path]:
    """Rewrite the service worker cache versions from the shipped asset content.

    The shell list is parsed out of the worker itself so the precache manifest
    stays the single source of truth. A content hash means a deploy always
    invalidates the offline shell without anyone remembering to bump a string.
    """
    worker_path = root / "service-worker.js"
    if not worker_path.is_file():
        return None
    text = worker_path.read_text("utf-8")
    shell_block = re.search(r"const SHELL_PATHS = \[(.*?)\];", text, re.DOTALL)
    if not shell_block:
        warnings.add("service-worker.js has no SHELL_PATHS list; cache versions were not stamped.")
        return None

    digest = hashlib.sha256()
    for relative in sorted(set(re.findall(r'"([^"]+)"', shell_block.group(1)))):
        if relative == "./":
            # Navigation alias for index.html, which is hashed under its own name.
            continue
        asset = root / relative
        if not asset.is_file():
            warnings.add(f"service-worker.js precaches missing asset: {relative}")
            continue
        digest.update(relative.encode("utf-8"))
        digest.update(asset.read_bytes())

    shell_version = f"spotterdex-shell-{digest.hexdigest()[:16]}"
    media_version = f"spotterdex-media-{hashlib.sha256(media_profile.encode('utf-8')).hexdigest()[:16]}"
    for constant, value in (("SHELL_CACHE_VERSION", shell_version), ("MEDIA_CACHE_VERSION", media_version)):
        pattern = rf'(const {constant} = ")[^"]*(")'
        if not re.search(pattern, text):
            warnings.add(f"service-worker.js has no {constant} declaration; cache version was not stamped.")
            continue
        text = re.sub(pattern, lambda match, value=value: f"{match.group(1)}{value}{match.group(2)}", text)

    return worker_path if write_text_if_changed(worker_path, text) else None


def latest_photo_date(photos: Iterable[Dict[str, Any]]) -> str:
    """Newest capture date in a collection, used for per-entity sitemap freshness."""
    dates = [
        str(photo.get("date") or "")
        for photo in photos
        if isinstance(photo, dict) and re.match(r"^\d{4}-\d{2}-\d{2}$", str(photo.get("date") or ""))
    ]
    return max(dates) if dates else ""


def resolve_photos(photo_ids: Iterable[str], photos_by_id: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [photos_by_id[photo_id] for photo_id in photo_ids if photo_id in photos_by_id]
...
def write_seo_files(
    ...
    for record in share_records:
        ...
        # Photos carry their own capture date; collections inherit the newest date
        # among their members so a page only claims to change when its content does.
        record_date = str(
            (record.get("date") if record["kind"] == "photo" else record.get("latestPhotoDate")) or ""
        )
        lastmod = record_date if re.match(r"^\d{4}-\d{2}-\d{2}$", record_date) else default_lastmod
    ...
    for loc, lastmod, priority in entries:
        ...
        lines.append("  </url>")
    lines.append("</urlset>")
    write_text_if_changed(sitemap_output, "\n".join(lines) + "\n")
    ...
    write_text_if_changed(robots_output, "\n".join(robots_lines))
        ...
        records.append(
            social_preview_record(
                ...
                extra={
                    "heading": type_name,
                    "count": len(photo_ids),
                    "latestPhotoDate": latest_photo_date(resolve_photos(photo_ids, photos_by_id)),
                    "countries": countries,
        ...
        records.append(
            social_preview_record(
                ...
                extra={
                    ...
                    "latestPhotoDate": latest_photo_date(pin_photos),
