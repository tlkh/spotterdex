...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-raw-assets",
        action="store_true",
        help="Validate schema, relationships and the SQL snapshot without checking that "
        "raw_assets/ sources exist. Required in CI, where raw_assets/ is not committed.",
    )
...
def main() -> int:
    ...
    raw_assets_dir: Path | None = None if args.skip_raw_assets else root / "raw_assets"
    if raw_assets_dir is not None and not raw_assets_dir.is_dir():
        raise SystemExit(
            f"Raw assets directory not found: {raw_assets_dir}\n"
            "raw_assets/ is intentionally not committed. Pass --skip-raw-assets to validate "
            "the catalog without checking source images."
        )
    ...
    try:
        ...
        if args.command == "reset-caption-markers":
            ...
            errors = validate_database(connection, raw_assets_dir=raw_assets_dir)
            if errors:
                connection.rollback()
                for error in errors:
                    print(f"error: {error}")
