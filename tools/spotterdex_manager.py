...
class SpotterDexManager:
    ...
    def wait_for_quality_scan(self, timeout: float = 120.0) -> Dict[str, Any]:
        """Return manager state once the background quality scan has settled.

        ``get_state`` starts the scan lazily and returns immediately, so the
        browser polls ``/api/quality-status``. Non-interactive callers such as
        tests have no poll loop, and need to block until results exist before
        using quality-gated actions.
        """
        ...
        while True:
            ...
            if status == "ready":
                return state
            if status == "error":
                raise ValueError(quality.get("message") or "Quality check failed.")
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                raise TimeoutError(f"Quality check did not finish within {timeout:g}s.")
            with self._quality_lock:
                worker = self._quality_thread
            if worker is not None and worker.is_alive():
                worker.join(min(remaining, 0.5))
            else:
                # The scan is queued but not yet observable on this thread.
