(function () {
      ...
      <div class="photo-viewer" id="photoViewer" role="dialog" aria-modal="true" aria-label="Photo viewer" hidden>
        ...
        <div class="viewer-stage">
          ...
          <div class="viewer-zoom-controls" aria-label="Photo display controls">
            ...
            <button class="viewer-button viewer-tool viewer-clean-toggle" type="button" id="viewerCleanButton" aria-pressed="false">Clean</button>
        ...
        <aside class="viewer-info" id="viewerInfo">
          <div class="viewer-info-sheet-bar">
            <span class="viewer-info-sheet-handle" aria-hidden="true"></span>
  ...
  function setupEvents() {
    ...
    if (els.viewSelect) {
      els.viewSelect.addEventListener("change", () => window.location.assign(els.viewSelect.value));
    }
  ...
  function handleDocumentClick(event) {
    ...
    const loadMoreButton = event.target.closest("[data-archive-load-more]");
    if (loadMoreButton) {
      if (loadMoreButton.dataset.archiveLoadMore === "aircraft") {
        state.dexVisibleCount += MOBILE_ARCHIVE_PAGE_SIZE;
        renderDex();
      } else if (loadMoreButton.dataset.archiveLoadMore === "squadrons") {
        state.squadronVisibleCount += MOBILE_ARCHIVE_PAGE_SIZE;
        renderSquadronsPage();
      }
      return;
  ...
  function renderMapSheetBar(title, panelLabel) {
    return `
      <div class="map-sheet-bar">
        <span class="map-sheet-handle" aria-hidden="true"></span>
  ...
  function renderSquadronsPage() {
    ...
    const visibleSquadrons = isMobile
      ? filteredSquadrons.slice(0, state.squadronVisibleCount)
      : filteredSquadrons;
  ...
  function renderArchivePagination(container, visibleCount, totalCount, kind, itemLabel) {
    ...
    container.innerHTML = `
      ...
      ${hasMore
        ? `<button class="archive-load-more" type="button" data-archive-load-more="${escapeAttr(kind)}">Show ${Math.min(MOBILE_ARCHIVE_PAGE_SIZE, totalCount - visibleCount)} more</button>`
        : ""}
    `;
  }
  ...
  function openViewer(photoId, context, options = {}) {
    ...
    hydrateFullPhotoData().then((wasUpdated) => {
      if (wasUpdated && !els.photoViewer.hidden) {
        renderViewerPhoto();
      }
    });
  ...
  function applyDeepLinkFromHash(options = {}) {
    ...
    const legacyDirectoryView = params.get("view");
    ...
    try {
      ...
      const hasLegacyRoute = Boolean(
        legacyDirectoryView
        || (aircraftId && pageViewId !== "dexView")
        || (aircraftFamily && pageViewId !== "dexView")
        || (squadronId && pageViewId !== "squadronsView")
        || (airshowId && pageViewId !== "airshowsView")
        || (locationId && pageViewId !== "mapView")
        || (statsSection && pageViewId !== "statsView")
      );
      if (hasLegacyRoute) {
        window.history.replaceState(
          { spotterdex: true, spotterdexKind: "directory" },
          "",
